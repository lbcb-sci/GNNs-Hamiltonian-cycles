...
def create_dataset_in_critical_regime(store_directory, sizes, nr_examples, hamilton_existence_probabilities):
    if not store_directory.exists():
        ...
        if len([x for x in store_directory.listdir() if x.suffix == ".pt"]) > 0:
...
if __name__ == '__main__':
    ...
    if not GRAPH_DATA_DIRECTORY_SUPERCRITICAL_CASE.exists():
    ...
    elif len([x for x in GRAPH_DATA_DIRECTORY_SUPERCRITICAL_CASE if x.suffix == ".pt"]) > 0:
        ...
        ErdosRenyiInMemoryDataset.create_dataset_from_edge_probabilities(
            ...
            sizes=[RESULTS_SIZE_GENERALIZATION_PLOT_DATASET_SIZES[0] for _ in RESULTS_SUPERCRITICAL_REGIME_ER_P_PARAM],
            nr_examples=[DATASET_SIZE_ACCURACY_VARIANCE_002_95PERC_CONF for _ in RESULTS_SUPERCRITICAL_REGIME_ER_P_PARAM],
            edge_existence_probability=RESULTS_SUPERCRITICAL_REGIME_ER_P_PARAM,
        )
        print(f"Completed.")
