...
def test_concorde_execution_time():
    graph_sizes = [int((1.5)**k) for k in range(9, 29)]
    ham_existence_prob = [0.1, 0.3, 0.5, 0.7, 0.95]
    examples_per_size = 5
    ...
    for size, ham_existence_prob in tqdm(list(itertools.product(graph_sizes, ham_existence_prob))):
