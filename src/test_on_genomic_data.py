...
def test_wandb_model_on_genomic_data(wandb_id: str, model:HamiltonSolver=None, dataset_summary_file=None, wandb_project=constants.WEIGHTS_AND_BIASES_PROJECT):
    try:
        ...
        stats, _ = test_on_genomic_dataset(model, dataset_summary_file)
        for key, value in stats:
            # wandb_run.log(f"genome_stats/{key}", value)
            print(f"genome_stats/{key}", value)
...
if __name__ == "__main__":
    ...
    stats, eval = test_wandb_model_on_genomic_data("", concorde_solver)
    print(stats)